...
export class Upload extends EventEmitter {
  ...
  private abortMultipartUploadCommand: AbortMultipartUploadCommand | null =
    null;
  ...
  constructor(options: Options) {
    ...
    this.leavePartsOnError =
      options.leavePartsOnError || this.leavePartsOnError;
  ...
  private async __uploadUsingPut(dataPart: RawDataPart): Promise<void> {
    ...
    const Location: string = (() => {
      const endpointHostnameIncludesBucket = endpoint.hostname.startsWith(
        `${locationBucket}.`
      );
  ...
  private async __createMultipartUpload(): Promise<CreateMultipartUploadCommandOutput> {
    const requestChecksumCalculation =
      await this.client.config.requestChecksumCalculation();
    if (!this.createMultiPartPromise) {
      const createCommandParams = { ...this.params, Body: undefined };
      if (requestChecksumCalculation === "WHEN_SUPPORTED") {
        createCommandParams.ChecksumAlgorithm =
          this.params.ChecksumAlgorithm || ChecksumAlgorithm.CRC32;
      }
  ...
  private async __doMultipartUpload(): Promise<CompleteMultipartUploadCommandOutput> {
    ...
    if (this.isMultiPart) {
      ...
      result = await this.client.send(
        new CompleteMultipartUploadCommand(uploadCompleteParams)
      );
      if (
        typeof result?.Location === "string" &&
        result.Location.includes("%2F")
      ) {
  ...
  private async markUploadAsAborted(): Promise<void> {
    if (
      this.uploadId &&
      !this.leavePartsOnError &&
      null !== this.abortMultipartUploadCommand
    ) {
